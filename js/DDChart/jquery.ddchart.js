	...
	jQuery.fn.ddBarChart = function(options) {
		...
		function buildYAxis (objSettings, strID) {
			...
			for (intValues = 0; intValues < objSettings.chartData.DATA.length; intValues++) {
                if(objSettings.chartData.DATA[intValues][objSettings.chartData.COLUMNS.indexOf('CONTEXT')] == "Perguntas"){
                    intYMax = 1;
                    context = "Perguntas";
                } else {
                    console.log(objSettings.chartData.DATA[intValues][objSettings.chartData.COLUMNS.indexOf('X_BAR_VALUE')]);
                    console.log(parseInt(objSettings.chartData.DATA[intValues][objSettings.chartData.COLUMNS.indexOf('X_BAR_VALUE')]));
                    var aux = objSettings.chartData.DATA[intValues][objSettings.chartData.COLUMNS.indexOf('X_BAR_VALUE')] - parseInt(objSettings.chartData.DATA[intValues][objSettings.chartData.COLUMNS.indexOf('X_BAR_VALUE')]);
                    if(aux > 0)
                        aux = objSettings.chartData.DATA[intValues][objSettings.chartData.COLUMNS.indexOf('X_BAR_VALUE')] + 1;
                    else
                        aux = objSettings.chartData.DATA[intValues][objSettings.chartData.COLUMNS.indexOf('X_BAR_VALUE')]
                        
                        intYMax = parseInt(aux) > intYMax ? parseInt(aux) : intYMax;  
            ...
            if(context == "Perguntas"){
                for (intYLabels = (intYMin+intYStep); intYLabels <= intYMax; intYLabels += intYStep) {
                    $(strID + " > .ddchart-y-axis-wrapper").append('<div class="ddchart-y-axis" style="bottom:'+((intYLabels-intYStep)/intYMax)*100+'%; height:'+(intYStep/intYMax)*100+'%; z-index:100; overflow:hidden;"><div style="display:inline">'+'Sim'+'</div></div>');
                }
            } else {
                    for (intYLabels = (intYMin+intYStep); intYLabels <= intYMax; intYLabels += intYStep) {
		...
		function buildChart (objSettings, strID, intYMax) {
			...
			for (intBars = 0; intBars < objSettings.chartData.DATA.length; intBars++) {
                ...
                if(objSettings.chartData.DATA[intBars][objSettings.chartData.COLUMNS.indexOf('X_BAR_VALUE')] == 0 && objSettings.chartData.DATA[intBars][objSettings.chartData.COLUMNS.indexOf('CONTEXT')] != "Perguntas"){
                    strBGColor = "background-color:#FFCCCC;";
                    objSettings.chartData.DATA[intBars][objSettings.chartData.COLUMNS.indexOf('X_BAR_VALUE')] = 0.5
                }
